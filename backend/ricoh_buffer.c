...
ricoh_buffer *
ricoh_buffer_create (SANE_Int  size,
                     SANE_Int  pixels_per_line,
                     SANE_Int  info_size,
                     SANE_Bool is_rgb)
{
  ...
  self = malloc (sizeof (ricoh_buffer));
  if (!self)
    return NULL;
...
/* destructor */
void
ricoh_buffer_dispose (ricoh_buffer *self)
{
  assert (self);
  free (self->data);
  free (self);
...
SANE_Byte *
ricoh_buffer_get_internal_buffer (ricoh_buffer *self)
{
  assert (self);
...
SANE_Int
ricoh_buffer_get_bytes_remain (ricoh_buffer *self)
{
  assert (self);
...
SANE_Int
ricoh_buffer_get_data (ricoh_buffer *self, SANE_Byte *dest, SANE_Int dest_size)
{
