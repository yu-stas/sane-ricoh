...
static Ricoh_Device *ricoh_devices = NULL;
...
lookup_handle(SANE_Handle handle)
{
  ...
  for (device = ricoh_devices; device; device = device->next)
    {
      if (device == handle)
        return device;
    }
...
attach (SANE_String_Const devname)
{
  ...
  device = (Ricoh_Device *) malloc (sizeof (Ricoh_Device));
  if (!device) 
    {
  ...
  device->next = ricoh_devices;
  ricoh_devices = device;
...
SANE_Status
sane_ricoh_aficio_init (SANE_Int *vc, SANE_Auth_Callback __sane_unused__ cb)
{    
  DBG_INIT ();

  DBG(2, "sane_ricoh_aficio_init\n");
  ...
  DBG(2, "sane_ricoh_aficio_init: end\n");
...
SANE_Status
sane_ricoh_aficio_get_devices (const SANE_Device ***dl,
                               SANE_Bool __sane_unused__ local)
{
  Ricoh_Device *device = NULL;
  SANE_Int i = 0;

  DBG(2, "sane_ricoh_aficio_get_devices\n");
...
SANE_Status
sane_ricoh_aficio_open (SANE_String_Const name, SANE_Handle *handle)
{
  Ricoh_Device *device;
  SANE_Status status;
...
sane_ricoh_aficio_get_option_descriptor (SANE_Handle handle, SANE_Int option)
{
  Ricoh_Device *device;
...
SANE_Status
sane_ricoh_aficio_control_option (SANE_Handle handle, 
                                  SANE_Int    option, 
                                  SANE_Action action,
                                  void       *value, 
                                  SANE_Word  *info)
...
static void 
update_scan_params (Ricoh_Device *device)
{
...
SANE_Status
sane_ricoh_aficio_get_parameters (SANE_Handle handle, SANE_Parameters *params)
{
  Ricoh_Device *device;
...
SANE_Status
sane_ricoh_aficio_start (SANE_Handle handle)
{
  Ricoh_Device *device;
  SANE_Status status;
  ...
  device->buffer =
      ricoh_buffer_create (MAX_LINE_SIZE,
                           pixels_per_line,
...
SANE_Status
sane_ricoh_aficio_read (SANE_Handle handle, 
                        SANE_Byte  *data, 
                        SANE_Int    maxlen, 
                        SANE_Int   *length)
{
  ...
  *length = ricoh_buffer_get_data (device->buffer,
                                   data,
  ...
  DBG (2,
       ...
       ricoh_buffer_get_bytes_remain(device->buffer));
...
SANE_Status
sane_ricoh_aficio_set_io_mode (SANE_Handle handle, SANE_Bool non_blocking)
{
  Ricoh_Device *device;
...
SANE_Status
sane_ricoh_aficio_get_select_fd (SANE_Handle handle, SANE_Int *fd)
{
  Ricoh_Device *device;
...
void
sane_ricoh_aficio_cancel (SANE_Handle handle)
{
  Ricoh_Device *device;
...
void
sane_ricoh_aficio_close (SANE_Handle handle)
{
  Ricoh_Device *device;

  DBG (2, ">sane_ricoh_aficio_close\n");
  ...
  ricoh_buffer_dispose(device->buffer);
  device->buffer = NULL;
  sanei_usb_exit();
...
void
sane_ricoh_aficio_exit (void)
{
  Ricoh_Device *device, *next;

  DBG (2, "sane_ricoh_aficio_exit\n");

  if (!initialized)
    return;

  for (device = ricoh_devices, next = device; device; device = next)
    {
      next = device->next;
      free (device);
    }
